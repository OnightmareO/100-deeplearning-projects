...
from model import SimpleWeatherClassifier
from earlystopping import EarlyStopping
weathers_dataset = WeatherDataset(root_dir='./data/weather_photos/', transform=None)
'''
#检查数据集
print(min(weather_dataset[0].size[0] for weather_dataset in weathers_dataset))
print(min(weather_dataset[0].size[1] for weather_dataset in weathers_dataset))
print(f'Length of the dataset: {len(weathers_dataset)}')
sel_idx = 10
img, label = weathers_dataset[sel_idx]
print(f'Label: {label}')
print(f'Description: {weathers_dataset.get_label_description(label)}')
# Print its shape
print(f'Image shape: {img.size}')  # PIL image size is (width, height)
'''

mean, std = get_mean_std(weathers_dataset)
main_transform, transform_with_augmentation = get_transformations(mean, std)
train_loader, val_loader, test_loader = get_dataloader(
    dataset=weathers_dataset,
    batch_size=32,
    val_part=0.1,
    test_part=0.1,
...
model = SimpleWeatherClassifier(num_classes=4).to(device)

criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(model.parameters(), lr=0.001)

early_stopper = EarlyStopping(patience=3, min_delta=0.001, verbose=True, path='./models/weather_best.pth', mode='min')
num_epochs = 10
...
model.load_state_dict(torch.load('./models/weather_best.pth'))
print("Loaded best model from ./models/weather_best.pth")
torch.save(model.state_dict(), './models/5.weather_classifier.pth')
print("Saved best model to ./models/5.weather_classifier.pth")
model.eval()
correct = 0
total = 0
with torch.no_grad():
