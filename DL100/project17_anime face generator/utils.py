...
def save_model(model,epoch,save_dir= './models/',model_name='17dcgan_face.pth'):
...
def load_model(model, model_dir='./models/', model_name='17dcgan_face.pth'):
...
def save_images(images, epoch, folder='./data/17_dcgan_animeface/', prefix='generated'):
...
def visualize_generated_images(generator,device, num_images=5):
    ...
    for idx in range(num_images):
        # 显示图像，并设置灰度色彩映射
        axes[idx].imshow(generated_images[idx].squeeze())
        # 关闭坐标轴
        axes[idx].axis('off')
    # 调整子图布局
    plt.tight_layout()
