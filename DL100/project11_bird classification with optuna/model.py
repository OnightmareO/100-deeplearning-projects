...
class FlexibleCNN(nn.Module):
    ...
    def _create_classifier(self, flattened_size, device, num_classes):
        self.classifier = nn.Sequential(
            nn.Dropout(self.dropout_rate),
            nn.Linear(flattened_size, self.fc_size),
            nn.ReLU(inplace=True),
            nn.Dropout(self.dropout_rate),
            nn.Linear(self.fc_size, num_classes)  # Assumes num_classes output classes
        ).to(device)
    ...
    def forward(self, x):
        ...
        if self.classifier is None:
            num_classes = 4  # 假设有4个类别，实际使用时请根据数据集调整
            self._create_classifier(flattened_size, device, num_classes)
